/* Author: Andrew, Dev, Sean
Date: 5/2/2024
Description: GameEngine class function definitions. GameEngine is the driver class of this program
*/
...
void GameEngine::initializeGame(){
	...
	initVeggies();
	initCaptain();
	timerTick(); // spawns rabbits
	//initSnake(); leaving this function to finish later for fun
...
void GameEngine::initVeggies(){
	...
	cout << "Please enter the name of the config file: ";
	cin >> filename;

	// Check if file exists
	file.open(filename);
	while (!file.is_open()) {
		cout << "Config file does not exist! Please enter the name of the config file: ";
		cin >> filename;
		file.open(filename);
	}
...
void GameEngine::moveRabbits(){
	for (int i = 0; i < rabbits.size(); i++) {
		...
		// make sure to check for rabbits so we dynamic cast against the spot we want to move to every time
		switch (direction) {
			case 0: // Up
	...
	switch (direction) {
		case 'w': // Up
			if (y > 0) {
				...
				cout << "Invalid move! You cannot move up. Captain Veggie ran into the fence and hit his head!" << endl;
			}
			break;
		case 's': // Down
			if (y < height - 1) {
				...
				cout << "Invalid move! You cannot move down. Captain Veggie ran into the fence and hit his head!" << endl;
			}
			break;
		case 'a': // Left
			if (x > 0) {
				...
				cout << "Invalid move! You cannot move left. Captain Veggie ran into the fence and hit his head!" << endl;
			}
			break;
		case 'd': // Right
			if (x < width - 1) {
				...
				cout << "Invalid move! You cannot move right. Captain Veggie ran into the fence and hit his head!" << endl;
			}
			break;
	}
}
/*
void GameEngine::moveSnakeHorizontal(int direction){
	int x = snake -> getX();
	int y = snake -> getY();
	int newX;
	switch (direction) {
...
*/
/*
void GameEngine::moveSnakeVertical(int direction){
...
void GameEngine::moveSnake(){
	int x = snake -> getX();
	int y = snake -> getY();
	int capX = captain -> getX();
	int capY = captain -> getY();
	if(capX > x){
	}
}
*/
